...
void TextConsole::SetWindowHWnd(const RECT& layout)
{
    std::lock_guard<std::mutex> lock(m_mutex);
...
void TextConsoleImage::SetWindowHWnd(const RECT& fullscreen, bool useSafeRect)
{
    m_fullscreen = fullscreen;

    if (useSafeRect)
    {
        TextConsole::SetWindowHWnd(
            SimpleMath::Viewport::ComputeTitleSafeArea(UINT(fullscreen.right - fullscreen.left),
                UINT(fullscreen.bottom - fullscreen.top)));
    }
    else
    {
        TextConsole::SetWindowHWnd(fullscreen);
    }
