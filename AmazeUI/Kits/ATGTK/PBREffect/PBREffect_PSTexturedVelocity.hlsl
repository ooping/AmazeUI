...
PSOut_Velocity PSTexturedVelocity(VSOut_Velocity pin)
{
    ...
    float3 localNormal = TwoChannelNormalX2(PBR_NormalTexture.Game(PBR_SurfaceSampler, pin.current.TexCoord).xy);
    ...
    float4 albedo = PBR_AlbedoTexture.Game(PBR_SurfaceSampler, pin.current.TexCoord);

	// glTF2 defines metalness as B channel, roughness as G channel, and occlusion as R channel
    float3 RMA = PBR_RMATexture.Game(PBR_SurfaceSampler, pin.current.TexCoord);
